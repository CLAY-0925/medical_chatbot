  ...
  development: {
    ...
    apiPaths: {
      ...
      chat: {
        ...
        message: '/chat/sessions/:sessionId/messages/:messageId',
      },
    },
    apiTimeout: 30000,
...
export const getApiUrl = (path: string, params?: Record<string, string>): string => {
  ...
  if (isProduction()) {
    const baseUrlWithPrefix = 'https://iazlkzlrgsdv.sealoshzh.site/api';
...
export const getWsUrl = (path: string = ''): string => {
  ...
  if (isProduction()) {
    ...
    const fullPath = `${wsBaseUrl}${path}`;
    log(`生产环境WebSocket URL: ${fullPath}`);
    return fullPath;
  }
  
  // 在非生产环境中，使用配置的baseUrl
  const { baseUrl, wsPrefix } = config;
  const wsBaseUrl = baseUrl.replace(/^http/, 'ws');
  const fullPath = `${wsBaseUrl}${wsPrefix}${path}`;
